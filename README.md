...
系统保留现有 User-CF 主链路，并在其外层增加“内容匹配 + 热门兜底”的轻量融合层：

```mermaid
flowchart LR
    A["学生画像\n求职意向 / 技能 / 城市 / 学历 / 收藏 / 投递"] --> D["混合排序"]
    B["岗位特征\n类别 / 城市 / 薪资 / 学历 / 技能 / 经验"] --> D
    C["User-CF\n相似用户收藏行为"] --> D
    E["热门岗位\nstoreupnum / clicktime"] --> D
    D --> F["推荐结果\nreason / scoreBreakdown"]
    F --> G["前端推荐卡\n推荐理由"]
    F --> H["离线评估\nPrecision@K / Recall@K / Coverage / Fallback Ratio"]
```

这样设计的原因是：老用户继续受益于相似用户行为，新用户或稀疏用户仍能通过内容特征与热门岗位获得可用推荐，同时接口返回可解释字段，便于前端展示与离线评估。
...
- 协同过滤分作为混合排序中的 `collaborative` 分，不直接覆盖内容匹配与热门兜底

对应实现可参考：

- `src/main/java/com/utils/UserBasedCollaborativeFiltering.java`
- `src/main/java/com/utils/HybridJobRecommender.java`
...
- 岗位特征：岗位类别、城市、薪资、学历要求、技能关键词、经验要求。
- 学生画像：求职意向、技能、城市、学历、收藏记录、投递记录。
- 融合得分：`total = collaborative * 0.50 + content * 0.35 + hot * 0.15`。
- 过滤策略：已收藏、已投递岗位不再作为个性化候选。
...
```bash
python scripts/evaluation/recommendation_metrics.py
```

示例输出：

```text
K=3 Precision=0.3333 Recall=0.5000 HitRate=1.0000
K=5 Precision=0.3333 Recall=0.5000 HitRate=1.0000
Coverage=0.7500 FallbackRatio=0.5000
```
...
- 推荐算法已接入轻量内容特征，后续可继续引入更强的语义向量匹配。
- 测试自动化程度可进一步提升（接口回归、CI 场景覆盖）。
- 默认示例数据可进一步精简并增强脱敏。
...
- [x] 引入用户画像与岗位内容特征融合推荐
- [x] 增加推荐效果评估指标（Precision@K / Recall@K / Coverage / Fallback Ratio）
- [ ] 引入 Redis 缓存与热点查询优化
- [ ] 完善 CI/CD 流水线
- [ ] 完成容器化部署模板
...
2. 再在基础业务之上补推荐能力，先实现冷启动热门推荐，再做基于收藏行为的协同过滤推荐，最后补内容匹配、解释字段和评估脚本。
...
- 当用户存在收藏行为后，走 `/zhaopinxinxi/autoSort2`，从 `storeup` 收藏记录中构建用户-岗位评分矩阵，再通过 `UserBasedCollaborativeFiltering` 计算相似用户分；
- 混合排序层融合协同过滤分、内容匹配分和热门兜底分，并返回 `reason`、`scoreBreakdown`；
...
- 将当前关键词匹配升级为岗位语义向量匹配，提升推荐精度；
- 增加点击率、投递转化率等线上效果评估指标；
